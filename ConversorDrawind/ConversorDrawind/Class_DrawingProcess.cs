...
namespace ConversorDrawind
{
    class Class_DrawingProcess
    {
        ...
        private static Class_DrawingProcess myClass = new Class_DrawingProcess();
            ...
            try
            {
                using (Class_MessageFilter.ScopedRegistration())
                {
                    ...
                    if (parametros.configuration.EXTCONFIsExchangeFormat)
                    {
                        ...
                        {
                            throw new ArgumentException("Não foi possível encontrar o formato atributado: " + parametros.configuration.PROGRAMblockFormatoCaminho);
                        }
                    }
                }
            }
